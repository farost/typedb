...
pub async fn initialise_system_database(server_state: &ServerState) -> Result<(), ArcServerStateError> {
    if server_state.databases().get_unrestricted(SYSTEM_DB).await?.is_some() {
        return Ok(());
    }
...
pub async fn initialise_default_user(server_state: &ServerState) -> Result<(), ArcServerStateError> {
    ...
    let exists = server_state.users().contains(accessor.clone(), DEFAULT_USER_NAME).await?;
    if !exists {
        server_state
            .users()
            .create(
