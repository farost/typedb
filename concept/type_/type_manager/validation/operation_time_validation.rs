...
use std::{collections::{HashMap, HashSet, VecDeque}, hash::Hash, iter, iter::once};
...
use lending_iterator::adaptors::Chain;
...
use crate::{
    ...
    type_::{
        ...
        constraint::{Constraint, ConstraintScope, filter_out_unchecked_constraints, get_checked_constraints},
...
use crate::type_::constraint::{CapabilityConstraint, ConstraintDescription, filter_by_constraint_category, filter_by_source, get_abstract_constraint, get_abstract_constraints, get_cardinality_constraint, get_cardinality_constraints, get_distinct_constraints, get_independent_constraints, get_range_constraints, get_regex_constraints, get_unique_constraint, get_unique_constraints, get_values_constraints, type_get_constraints_closest_source, TypeConstraint};
use crate::type_::{OwnerAPI, PlayerAPI};
use crate::type_::annotation::{AnnotationKey, AnnotationUnique};
...
macro_rules! for_type_and_subtypes_transitive {
    ($snapshot:ident, $type_manager:ident, $type_:ident, $closure:expr) => {
        let subtypes = $type_.get_subtypes_transitive($snapshot, $type_manager).map_err(SchemaValidationError::ConceptRead)?;
        TypeAPI::chain_types($type_, subtypes.into_iter()).try_for_each(|subtype| $closure(subtype))?;
    };
}
...
macro_rules! new_acquired_capability_instances_validation {
    ($func_name:ident, $capability_type:ident, $validation_func:path) => {
        pub(crate) fn $func_name(
            ...
            let affected_object_types = TypeAPI::chain_types(capability.object(), capability.object().get_subtypes_transitive(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?).collect();
            let affected_interface_types = TypeAPI::chain_types(capability.interface(), capability.interface().get_subtypes_transitive(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?).collect();
...
macro_rules! new_annotation_constraints_compatible_with_capability_instances_validation {
    ($func_name:ident, $capability_type:ident, $validation_func:path) => {
        pub(crate) fn $func_name(
            ...
            let affected_object_types = TypeAPI::chain_types(capability.object(), capability.object().get_subtypes_transitive(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?).collect();
            let affected_interface_types = TypeAPI::chain_types(capability.interface(), capability.interface().get_subtypes_transitive(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?).collect();
            let constraints = get_checked_constraints(annotation.to_capability_constraints(capability.clone()).into_iter());
...
macro_rules! updated_constraints_compatible_with_capability_instances_on_object_supertype_change_validation {
    ($func_name:ident, $capability_type:ident, $object_type:ident, $validation_func:path) => {
        pub(crate) fn $func_name(
            ...
            let affected_object_types = TypeAPI::chain_types(capability.object(), capability.object().get_subtypes_transitive(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?).collect();
            let new_capabilities = new_supertype.get_capabilities::<$capability_type>(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?;

            for new_capability in new_capabilities.into_iter() {
                let affected_interface_types = TypeAPI::chain_types(new_capability.interface(), new_capability.interface().get_subtypes_transitive(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?).collect();
                let constraints = get_checked_constraints(new_capability.get_constraints(snapshot, type_manger).map_err(SchemaValidationError::ConceptRead)?.into_iter());
...
macro_rules! affected_constraints_compatible_with_capability_instances_on_interface_subtype_unset_validation {
    ($func_name:ident, $capability_type:ident, $validation_func:path) => {
        pub(crate) fn $func_name(
            ...
            type_manager: &TypeManager,
            thing_manager: &ThingManager,
            interface_type: $capability_type<'static>::InterfaceType,
        ) -> Result<(), SchemaValidationError> {
            ...
            let mut objects_with_interface_supertypes_and_constraints: HashMap<$capability_type<'static>::ObjectType, HashSet<CapabilityConstraint<$capability_type<'static>>>> = HashMap::new();
            ...
            let mut objects_with_interface_subtypes_and_constraints: HashSet<$capability_type<'static>::ObjectType> = HashSet::new();
            collect_object_types_with_interface_type_and_subtypes(
                snapshot,
                type_manager,
                interface_type,
...
macro_rules! affected_constraints_compatible_with_capability_instances_on_interface_supertype_change_validation {
    ($func_name:ident, $capability_type:ident, $unset_validation_func:path, $validation_func:path) => {
        pub(crate) fn $func_name(
            ...
            thing_manager: &ThingManager,
            interface_type: $capability_type<'static>::InterfaceType,
            interface_supertype: $capability_type<'static>::InterfaceType,
        ) -> Result<(), SchemaValidationError> {
            ...
            let mut objects_with_interface_subtypes_and_constraints: HashSet<$capability_type<'static>::ObjectType> = HashSet::new();
            let all_interface_subtypes = collect_object_types_with_interface_type_and_subtypes(
                snapshot,
                type_manager,
                interface_type,
                &mut objects_with_interface_subtypes_and_constraints,
            ...
            for (affected_object_type, constraints) in objects_with_interface_supertypes_and_constraints.into_iter() {
                ...
                $validation_func(
                    ...
                    thing_manager,
                    &HashSet::from([affected_object_type]),
                    &all_interface_supertypes.into_iter().chain(all_interface_subtypes.into_iter()).collect(),
...
macro_rules! new_annotation_constraints_compatible_with_type_and_sub_instances_validation {
    ($func_name:ident, $type_:ident, $validation_func:path) => {
        pub(crate) fn $func_name(
            ...
            let affected_types = TypeAPI::chain_types(type_.clone(), type_.get_subtypes_transitive(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?).collect();
...
macro_rules! updated_constraints_compatible_with_type_and_sub_instances_on_supertype_change_validation {
    ($func_name:ident, $type_:ident, $validation_func:path) => {
        pub(crate) fn $func_name(
            ...
            let constraints = get_checked_constraints(new_supertype.get_constraints(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?.into_iter());
...
fn collect_object_types_with_interface_type_and_subtypes<CAP: Capability<'static>>(
    ...
    let all_interface_subtypes = TypeAPI::chain_types(interface_type, interface_subtypes.into_iter()).collect();

    for interface_subtype in &all_interface_subtypes {
        out_object_types.extend(
            TypeReader::get_object_types_with_capabilities_for_interface::<CAP>(
                snapshot,
                interface_subtype.clone()
            )
                .map_err(SchemaValidationError::ConceptRead)?
                .into_keys()
        );
...
fn collect_object_types_with_interface_type_and_supertypes_constraints<CAP: Capability<'static>>(
    ...
    let all_interface_supertypes = TypeAPI::chain_types(interface_type, interface_supertypes.into_iter()).collect();

    for interface_supertype in &all_interface_supertypes {
        for object_type_with_interface_supertype in TypeReader::get_object_types_with_capabilities_for_interface::<CAP>(
            snapshot,
...
impl OperationTimeValidation {
    ...
    pub(crate) fn validate_no_subtypes_for_type_deletion<T: KindAPI<'static>>(
        ...
        if no_subtypes {
            ...
            Err(SchemaValidationError::CannotDeleteTypeWithExistingSubtypes(get_label_or_schema_err(snapshot, type_manager, type_)?))
        }
    }
    ...
    pub(crate) fn validate_no_attribute_subtypes_to_unset_abstractness(
        ...
        let no_subtypes = attribute_type.get_subtypes(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?.is_empty();
        if no_subtypes {
            Ok(())
        } else {
            Err(SchemaValidationError::CannotUnsetAbstractnessOfAttributeTypeAsItHasSubtypes(get_label_or_schema_err(
                snapshot, type_manager, attribute_type,
            )?))
        }
    }
    ...
    fn validate_role_names_compatible_with_new_relation_supertype(
        ...
        let relation_supertype_supertypes = relation_supertype.get_supertypes_transitive(snapshot, type_manager)
            .map_err(SchemaValidationError::ConceptRead)?;
        let affected_supertypes = TypeAPI::chain_types(relation_supertype, relation_supertype_supertypes.into_iter());
        ...
        for subtype_relates in subtype_relates_declared {
            ...
            affected_supertypes
                .clone()
                .try_for_each(|supertype| validate_role_name_uniqueness_non_transitive(snapshot, type_manager, supertype.clone(), &role_label))?;
        }
    ...
    pub(crate) fn validate_value_type_compatible_with_abstractness(
        ...
        let is_abstract = abstract_set.unwrap_or(attribute_type.is_abstract(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?);
    ...
    pub(crate) fn validate_type_regex_narrows_supertype_constraints(
        ...
        if let Some(supertype) = attribute_type.get_supertype(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)? {
            let supertype_constraints = supertype.get_constraints_regex(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?;

            for supertype_constraint in supertype_constraints.into_iter() {
                supertype_constraint.validate_narrowed_by_strictly_same_type(&constraint_description).map_err(|source| SchemaValidationError::SubtypeConstraintDoesNotNarrowSupertypeConstraint(
    ...
    pub(crate) fn validate_type_range_narrows_supertype_constraints(
        ...
        if let Some(supertype) = attribute_type.get_supertype(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)? {
            let supertype_constraints = supertype.get_constraints_range(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?;
    ...
    pub(crate) fn validate_type_values_narrows_supertype_constraints(
        ...
        if let Some(supertype) = attribute_type.get_supertype(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)? {
            let supertype_constraints = supertype.get_constraints_values(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?;
    ...
    pub(crate) fn validate_subtypes_narrow_regex(
        ...
        let constraint_description = ConstraintDescription::Regex(regex);
        let subtypes = attribute_type.get_subtypes_transitive(snapshot, type_manager)
            .map_err(SchemaValidationError::ConceptRead)?;
        for subtype in subtypes.into_iter() {
            let regex_constraints = subtype.get_constraints_regex(snapshot, type_manager)
                .map_err(SchemaValidationError::ConceptRead)?;
            for regex_constraint in regex_constraints {
                regex_constraint.validate_narrows_strictly_same_type(&constraint_description)
                    .map_err(|source| SchemaValidationError::SubtypeConstraintDoesNotNarrowSupertypeConstraint(
    ...
    pub(crate) fn validate_subtypes_narrow_range(
        ...
        let subtypes = attribute_type.get_subtypes_transitive(snapshot, type_manager)
            .map_err(SchemaValidationError::ConceptRead)?;
        for subtype in subtypes.into_iter() {
            let range_constraints = subtype.get_constraints_range(snapshot, type_manager)
                .map_err(SchemaValidationError::ConceptRead)?;
            for range_constraint in range_constraints {
                range_constraint.validate_narrows_strictly_same_type(&constraint_description)
                    .map_err(|source| SchemaValidationError::SubtypeConstraintDoesNotNarrowSupertypeConstraint(
                        get_label_or_schema_err(snapshot, type_manager, subtype.clone())?,
                        get_label_or_schema_err(snapshot, type_manager, attribute_type.clone())?,
                        source,
    ...
    pub(crate) fn validate_subtypes_narrow_values(
        ...
        let subtypes = attribute_type.get_subtypes_transitive(snapshot, type_manager)
            .map_err(SchemaValidationError::ConceptRead)?;
        for subtype in subtypes.into_iter() {
            let values_constraints = subtype.get_constraints_regex(snapshot, type_manager)
                .map_err(SchemaValidationError::ConceptRead)?;
            for values_constraint in values_constraints {
                values_constraint.validate_narrows_strictly_same_type(&constraint_description)
                    .map_err(|source| SchemaValidationError::SubtypeConstraintDoesNotNarrowSupertypeConstraint(
                        get_label_or_schema_err(snapshot, type_manager, subtype.clone())?,
                        get_label_or_schema_err(snapshot, type_manager, attribute_type.clone())?,
                        source,
        ...
        for owns in all_owns.into_iter() {
            ...
            for regex_constraint in regex_constraints {
                regex_constraint.validate_narrows_strictly_same_type(type_constraint_description)
                    .map_err(|source| SchemaValidationError::CannotSetAnnotationToInterfaceBecauseItsConstraintIsNotNarrowedByItsCapabilityConstraint(
                        get_label_or_schema_err(snapshot, type_manager, attribute_type.clone())?,
                        source,
                    ))?;
            }
    ...
    pub(crate) fn validate_capability_regex_constraint_narrows_interface_type_constraints(
        ...
        let attribute_type_regex_constraints = owns.attribute().get_constraints_regex(snapshot, type_manager)
            .map_err(SchemaValidationError::ConceptRead)?;
        for regex_constraint in attribute_type_regex_constraints {
            regex_constraint.validate_narrowed_by_strictly_same_type(&capability_constraint_description)
                .map_err(|source| SchemaValidationError::CannotSetAnnotationToCapabilityBecauseItsConstraintDoesNotNarrowItsInterfaceConstraint(
    ...
    pub(crate) fn validate_capability_range_constraint_narrows_interface_type_constraints(
        ...
        let attribute_type_range_constraints = owns.attribute().get_constraints_range(snapshot, type_manager)
            .map_err(SchemaValidationError::ConceptRead)?;
    ...
    pub(crate) fn validate_capability_values_constraint_narrows_interface_type_constraints(
        ...
        let attribute_type_values_constraints = owns.attribute().get_constraints_values(snapshot, type_manager)
            .map_err(SchemaValidationError::ConceptRead)?;
    ...
    pub(crate) fn validate_attribute_type_supertype_is_abstract<T: KindAPI<'static>>(
        ...
        if type_.is_abstract(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)? {
            ...
            Err(SchemaValidationError::AttributeTypeSupertypeIsNotAbstract(get_label_or_schema_err(snapshot, type_manager, type_)?))
        }
    }
    ...
    // TODO: Capabilities constraints narrowing checks are currently disabled
    pub(crate) fn validate_cardinality_of_inheritance_line_with_updated_interface_type_supertype<CAP: Capability<'static>>(
        snapshot: &impl ReadableSnapshot,
        type_manager: &TypeManager,
        interface_type: CAP::InterfaceType,
        new_supertype: Option<CAP::InterfaceType>,
    ) -> Result<(), SchemaValidationError> {
        // TODO: Implement
    ...
    pub(crate) fn validate_updated_owns_does_not_conflict_with_any_sibling_owns_ordering(
        ...
        let owner_subtypes = owns.owner().get_subtypes_transitive(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?;
        let affected_owner_types = TypeAPI::chain_types(owns.owner(), owner_subtypes.into_iter());

        for owner_type in affected_owner_types {
            validate_sibling_owns_ordering_match_for_type(
                snapshot,
                type_manager,
                owner_type,
                &updated_owns,
    ...
    pub(crate) fn validate_relates_distinct_constraint_ordering(
        ...
        let distinct_set = distinct_set.unwrap_or(relates.is_distinct(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?);

        if Self::is_ordering_compatible_with_distinct_constraint(ordering, distinct_set) {
            Ok(())
        } else {
            Err(SchemaValidationError::InvalidOrderingForDistinctConstraint(get_label_or_schema_err(snapshot, type_manager, role)?))
        }
    }
    ...
    pub(crate) fn validate_owns_distinct_constraint_ordering(
        ...
        let ordering = ordering.unwrap_or(owns.get_ordering(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?);
        let distinct_set = distinct_set.unwrap_or(owns.is_distinct(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?);

        if Self::is_ordering_compatible_with_distinct_constraint(ordering, distinct_set) {
            Ok(())
        } else {
            Err(SchemaValidationError::InvalidOrderingForDistinctConstraint(get_label_or_schema_err(
                snapshot, type_manager, attribute,
            )?))
        }
    }
    ...
    pub(crate) fn validate_role_supertype_ordering_match(
        ...
        validate_role_type_supertype_ordering_match(snapshot, type_manager, subtype_role, supertype_role, set_subtype_role_ordering)
    }
    ...
    pub(crate) fn validate_sub_does_not_create_cycle<T: KindAPI<'static>>(
        ...
        let existing_supertypes = supertype.get_supertypes_transitive(snapshot, type_manager)
            .map_err(SchemaValidationError::ConceptRead)?;
    ...
    pub(crate) fn validate_relates_is_inherited(
        ...
        let super_relation = relation_type.get_supertype(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?;
        if let Some(super_relation) = super_relation {
            let is_inherited = super_relation.get_relates(snapshot, type_manager)
                .map_err(SchemaValidationError::ConceptRead)?
                .iter()
    ...
    pub(crate) fn validate_value_type_compatible_with_inherited_value_type(
        ...
        let inherited_value_type_with_source = attribute_type.get_value_type(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?;
    ...
    pub(crate) fn validate_subtypes_value_types_compatible_with_new_value_type(
        ...
        let subtypes = attribute_type.get_subtypes_transitive(snapshot, type_manager)
            .map_err(SchemaValidationError::ConceptRead)?;
        for subtype in subtypes.into_iter() {
            if let Some(subtype_value_type) = subtype.get_value_type_declared(snapshot, type_manager)
                .map_err(SchemaValidationError::ConceptRead)?
            {
    ...
    fn validate_when_attribute_type_loses_value_type(
        ...
        match value_type {
            Some(_) => {
                ...
                Self::validate_value_type_compatible_with_all_owns_annotations(snapshot, type_manager, attribute_type.clone(), None)?;
    ...
    pub(crate) fn validate_attribute_type_does_not_lose_instances_with_independent_constraint_with_new_supertype(
        ...
        let subtype_constraint_source =
            type_get_constraints_closest_source(snapshot, type_manager, attribute_type.get_constraints_independent(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?);
        let supertype_constraint_source = match &new_supertype {
            None => None,
            Some(new_supertype) => {
                type_get_constraints_closest_source(snapshot, type_manager, new_supertype.get_constraints_independent(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?)
            }
        };
                ...
                if subtype_source != attribute_type {
                    ...
                    for_type_and_subtypes_transitive!(snapshot, type_manager, attribute_type, |type_: AttributeType<'static>| {
                        let sub_subtype_constraint_source = type_get_constraints_closest_source(
                            snapshot,
                            type_manager,
                            type_.get_constraints_independent(snapshot, type_manager).map_err(SchemaValidationError::ConceptRead)?,
                        );
                        match sub_subtype_constraint_source {
                            ...
                            Some((_, sub_subtype_constraint_source)) => {
    ...
    pub(crate) fn validate_unset_owns_is_not_inherited(
        ...
        let all_owns = owner.get_owns(snapshot, type_manager)
            .map_err(SchemaValidationError::ConceptRead)?;
    ...
    pub(crate) fn validate_unset_plays_is_not_inherited(
        ...
        let all_plays = player.get_plays(snapshot, type_manager)
                .map_err(SchemaValidationError::ConceptRead)?;
    ...
    pub(crate) fn validate_declared_capability_annotation_is_compatible_with_declared_annotations<CAP: Capability<'static>>(
        snapshot: &impl ReadableSnapshot,
        type_manager: &TypeManager,
        capability: CAP,
        annotation_category: AnnotationCategory,
    ...
    pub(crate) fn validate_no_instances_to_delete<'a>(
        ...
        if has_instances {
            Err(SchemaValidationError::CannotDeleteTypeWithExistingInstances(get_label_or_schema_err(snapshot, type_manager, type_)?))
        } else {
            Ok(())
        }
    }
    ...
    pub(crate) fn validate_no_role_instances_to_set_ordering(
        ...
        if has_instances {
            Err(SchemaValidationError::CannotSetRoleOrderingWithExistingInstances(get_label_or_schema_err(
                snapshot, type_manager, role_type,
            )?))
        } else {
            Ok(())
        }
            ...
            if let Some(_) = entity_iterator.next().transpose().map_err(DataValidationError::ConceptRead)? {
                for abstract_constraint in abstract_constraints.iter() {
                    ...
                    if &abstract_constraint.source() == entity_type {
                        return Err(DataValidation::create_data_validation_entity_type_abstractness_error(&abstract_constraint));
                    }
                }
            }
        }
            ...
            if let Some(_) = relation_iterator.next().transpose().map_err(DataValidationError::ConceptRead)? {
                for abstract_constraint in abstract_constraints.iter() {
                    ...
                    if &abstract_constraint.source() == relation_type {
                        return Err(DataValidation::create_data_validation_relation_type_abstractness_error(&abstract_constraint));
                    }
                }
            }
        }
    ...
    fn validate_attribute_type_instances_against_constraints(
        ...
        for attribute_type in attribute_types {
            let mut attribute_iterator = thing_manager.get_attributes_in(snapshot, attribute_type.clone()).map_err(DataValidationError::ConceptRead)?;
            while let Some(attribute) = attribute_iterator.next() {
                ...
                for abstract_constraint in abstract_constraints.iter() {
                    ...
                    if &abstract_constraint.source() == attribute_type {
                        return Err(DataValidation::create_data_validation_attribute_type_abstractness_error(&abstract_constraint));
                    }
                }
    ...
    fn validate_role_type_instances_against_constraints(
        snapshot: &impl ReadableSnapshot,
        thing_manager: &ThingManager,
        role_types: &HashSet<RoleType<'static>>,
                ...
                while let Some(attribute) = has_attribute_iterator.next() {
                    ...
                    for abstract_constraint in abstract_constraints.iter() {
                        ...
                        if &abstract_constraint.source() == &owns {
                            return Err(DataValidation::create_data_validation_owns_abstractness_error(&abstract_constraint, object.as_reference()));
                        }
                    }
                ...
                while let Some(relation) = relations_iterator.next() {
                    ...
                    for abstract_constraint in abstract_constraints.iter() {
                        ...
                        if &abstract_constraint.source() == &plays {
                            return Err(DataValidation::create_data_validation_plays_abstractness_error(&abstract_constraint, object.as_reference()));
                        }
                    }
                ...
                while let Some(role_players) = role_players_iterator.next() {
                    ...
                    for abstract_constraint in abstract_constraints.iter() {
                        ...
                        if &abstract_constraint.source() == &relates {
                            return Err(DataValidation::create_data_validation_relates_abstractness_error(&abstract_constraint, relation.as_reference()));
                        }
                    }
    ...
    fn get_lost_capabilities_if_supertype_is_changed<CAP: Capability<'static>>(
        ...
        let current_capabilities =
            TypeReader::get_capabilities::<CAP>(snapshot, type_.clone(), false).map_err(SchemaValidationError::ConceptRead)?;
